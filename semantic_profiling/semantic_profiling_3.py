...
def semantic_profiling(path, start_time, n):  #function to do semantic profiling
    ...
    def stat1(df, c, n, function_list):        #inner function which gives the stats
    ...
    for filename in column_list:        #for each dataset
        if i <= n:
            ...
            k = stat1(df2, "_c0", 0, [])                         #check which functions can be applied for the 100 rows
            
            t['column_name'] = c
            t['semantic_types'] = stat1(df, "_c0", 1, k)       #process the entire dataset
